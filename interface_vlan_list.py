...
#    netbios = re.search(r"The network name.*:.*?([\S\s].*)\s", str).group(1)
    ...
    #    i += 1
    #
    vlanif = re.findall(r" (Loopback.*|Tunnel.*|GigabitEthernet.*|Vlan.*|Fast.*|Serial.*)\s*.*(nameif.*|description.*)[\s\S]{0,115}(ip address.*)\s", str)
    if vlanif != []:
...
wb.save('C:/python/outputdir/interface_vlan_list.xls')
print('wb saved')
